...
public class ExampleInstrumentedTest {
    ...
    @Test
    public void TestMyRoom() throws Exception {
        myRoom room = new myRoom("room1",1,10,1,1);

        // Testing if autogenerated id key is set and insert() + loadById()
        roomDao.insert(room);
        room = roomDao.loadById(1);
        assertEquals(1, room.getId());
        assertEquals("room1", room.roomName);

        // Testing update()
        room.setRoomName("room2");
        roomDao.update(room);
        room = roomDao.loadById(1);
        assertEquals("room2", room.getRoomName());
